...
class PickPlaceEnv():
  ...
  def reset(self, config):
    ...
    #zone_size = self.get_random_size(0.1, 0.25, 0.1, 0.25, 0.025, 0.025)
    zone_size = self.get_size(0.15,0.15, 0.025)
    block_x = [0.025, 0.025, 0.025, 0.0125, 0.0125, 0.0125]
    block_y = [0.025, 0.025, 0.0125, 0.025, 0.0125, 0.0125]
    size_num = 0

    for obj_name in obj_names:
      if ('block' in obj_name) or ('container' in obj_name) or ('box' in obj_name):

        # Get random position 15cm+ from other objects.
        while True:
          if obj_name == 'container':
            rand_xyz = np.float32([0, -0.5, 0.03]).reshape(1, 3)
          else:
        ...
        if len(parts) > 1:
          ...
          object_color = COLORS['brown']
          object_type = obj_name.split(' ')[0]

        object_position = rand_xyz.squeeze()

        if object_type == 'block':
          # block_size_x = random.uniform(0.015, 0.03)  # 3cm ~ 6cm (halfExtents)
          # block_size_y = 0.045 - block_size_x
          block_size_x = block_x[size_num]
          block_size_y = block_y[size_num]
          size_num += 1
          object_shape = pybullet.createCollisionShape(pybullet.GEOM_BOX, halfExtents=[block_size_x, block_size_y, 0.02])
          object_visual = pybullet.createVisualShape(pybullet.GEOM_BOX, halfExtents=[block_size_x, block_size_y, 0.02])
          object_id = pybullet.createMultiBody(0.01, object_shape, object_visual, basePosition=object_position)
        if object_type == 'container':
          # Add container box.
          container_template = 'bowl/container-template.urdf'
          half = np.float32(zone_size) / 2
          replace = {'DIM': zone_size, 'HALF': half}
          container_urdf = self.fill_template(container_template, replace)
          object_id = pybullet.loadURDF(container_urdf, object_position, useFixedBase=1)
        if object_type == 'box':
          # Add container box.
          container_template = 'bowl/container-template.urdf'
          half = np.float32(zone_size) / 2
  ...
  def step_error(self, action=None):
    ...
    # Place down object.
    while (not self.gripper.detect_contact()) and (pick_xyz[2] > 0.1):
      pick_xyz[2] -= 0.001
      self.movep(pick_xyz)
      for _ in range(3):
        self.step_sim_and_render()
...
# Define and reset environment.
config = {'pick':  ['purple block', 'blue block', 'blue block', 'blue block', 'blue block', 'blue block'],
          'place': ['container']}
# config = {'pick':  ['blue block', 'green block', 'yellow block'],
#           'place': ['brown box']}
...
def translate_to_eng(user_input):
    ...
    chat = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0, convert_system_message_to_human=True)
    ...
    # ✅ 실행
    eng_input = chain.run(
      input_language="korean",  # "korea" → "korean"으로 바꾸는 게 더 정확
...
class CustomCLIPort(LLM):
    ...
    def _call(
        ...
        img = env.get_camera_image()
        imageio.imwrite(f'before_action_{CLIPort_loop}.jpg', img)

        #error 발생
        dial = pyautogui.confirm('오류를 발생시키겠습니까?', buttons=['Y', 'N'])

        if dial == 'Y':
          obs = run_cliport_error(obs, task_plan_array[CLIPort_loop])
        else:
          obs = run_cliport(obs, task_plan_array[CLIPort_loop])

        img = env.get_camera_image()
        imageio.imwrite(f'after_action_{CLIPort_loop}.jpg', img)
...
class CustomValid(LLM):
    ...
    def _call(
        ...
        llm_valid = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0)
...
tools = [
    Tool(
      ...
      description="""한국어가 입력되면 번역을 위해 항상 이 tool을 첫 번째로 실행해야 한다. 다음 tool로 VILD를 실행시킬 것이다."""
    ),
    Tool(
        name = "VILD",
        func=VILD_chain.run,
    ...
    Tool(
        ...
        description= """난 tool을 사용해야만 한다. 난 final answer를 알아서는 안된다. 다음 tool로 VILD를 실행해야만 한다. 예를 들어 만약 observation이
        '다음 tool로 VILD tool을 실행 시켜야해.'라면, 다음 tool로 VILD를 실행해야만 한다."""
    ),
    Tool(
        name = "State_checker",
...
prefix = """당신은 도구를 순서대로 실행하는 에이전트입니다.
당신은 환경의 상태를 파악할 수 없으므로 가상의 환경을 마련하려고 하지 마십시오. 각 도구들이 대신 환경을 파악하고 작업을 수행해줄 것입니다.
도구들의 작동 순서에는 오류가 없습니다. 당신은 오로지 요청받는 것에 맞는 도구를 실행하는 것만 하시면 됩니다.
해당 프로그램은 주어진 문장을 통해 Planner 도구로 계획을 세우고 해당 계획을 단위 작업으로 나눕니다.
이후 각각의 단위 작업들을 하나씩 처리하기 위해 CLIPort, VILD, State_checker 순서대로 도구들을 반복적으로 사용합니다.
State_checker 도구 이후에 CLIPort 도구를 부르는 것은 이 다음의 단위 작업을 실행하는 행동이므로 정상적인 작동입니다.
만약 State_checker 도구에서 오류를 검출했다면, Planner 도구를 호출하여 계획을 세우는 것부터 다시 시작하게 됩니다.
모든 단위 작업이 모두 완료된 이후에는 "작업 종료!!!"라는 답변이 나오므로, 해당 답변을 받기 전까지는 임의로 작업을 종료하려고 시도하지 마십시오.
그리고, VILD tool을 요청받았다면 VILD tool을, CLIPort tool을 요청받았다면 CLIPort tool을 제대로 실행시켜줘야 합니다. 요청받은 도구가 아닌 다른 도구를 실행하려고 하지 마십시오.

아래 도구들을 사용할 수 있습니다:

- translate_to_eng: 한국어를 영어로 번역
- VILD: 이미지로부터 객체를 이해하고 추론
- Planner: 다음 작업 계획 수립
- CLIPort: 로봇 조작 실행
- State_checker: 현재 상태 점검
- Validation: 결과 검증
