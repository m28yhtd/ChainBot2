...
from test_task.test_task_mbb.task_4 import initial_prompt, initial_state, found_objects
plan_path = "task_plan/Task4.xml"

# np.random.seed(42)
# obs = env.reset(config)
import json
with open("init_envs/task4_2.json", "r") as f:
    config = json.load(f)
...
from prompt.Verification_Prompt import validation_sim_log
from prompt.Replanning_Prompt_1 import REPLANNING_PROMPT
from prompt.Replanning_Prompt_2 import REPLANNING_PROMPT2
...
def plan_execution_node(state: SystemState) -> SystemState:
  """
  GPT-5 기반 오류 탐지 및 상태 묘사 생성 + 행동 모듈
  """

  # TODO: GPT-5 호출 + XML 분석
  # 오류 탐지 모듈
  while(state["step_index"] <= len(state["apack"])):
    ...
    # 행동 모듈
    if state["step_index"] < len(state["apack"]):
      ...
      if error_trigger(error_rate):
        ...
        # obs = run_cliport_error2(obs, action_text) # 제자리 오류 발생
      else:
        print("정상 작동을 수행합니다.")
...
def planning_node(state: SystemState) -> SystemState:
    genai.configure(api_key=GOOGLE_API_KEY)
    model = genai.GenerativeModel("gemini-2.0-flash")
    ...
    if state["plan_mode"] == "create":
      print("[planning] 계획 생성")
      ...
      state["prev_plan"] = xml_str

    elif state["plan_mode"] == "revise":
      print("[replanning] 재계획")
...
def validation_node(state: SystemState) -> SystemState:
    ...
    # ------------------------
    # 1. Initial State Parser
    # ------------------------
    def parse_initial_state(initial_state: str):
      placements = {}
      top_status = {}
      ...
      for line in initial_state.strip().split("\n"):
        ...
        # Case 1: placement
        m = re.match(r"The (.+) is placed on top of the (.+)\.", line)
        if m:
          block, support = m.groups()
          placements[block] = support
          continue

        # Case 2: top status
        m = re.match(r"The (.+) is (not )?empty on top\.", line)
        if m:
          block, not_empty = m.groups()
          top_status[block] = not not_empty  # True if empty, False if not empty
          continue

      return placements, top_status

    # ------------------------
    # 2. Apply a Single apack
    # ------------------------
    def apply_apack(apack_elem, placements, top_status):
      """
      Execute one <apack> action on the current state.
      Return (updated placements, updated top_status, feasible, reason)
      """
      # Extract action line
      action_text = apack_elem.find("action").text.strip()
      m = re.match(r'robot\.pick_and_place\("(.+)", "(.+)"\)', action_text)
      if not m:
        return placements, top_status, False, "Invalid action format"
      pick, place = m.groups()

      # Feasibility: pick must be empty
      if not top_status.get(pick, True):
        return placements, top_status, False, f"Pick infeasible: {pick} is not empty on top."

      # Feasibility: place must be empty if it's a block
      if place in top_status and not top_status[place]:
        return placements, top_status, False, f"Place infeasible: {place} is not empty on top."

      # ✅ Update state
      new_placements = placements.copy()
      new_top_status = top_status.copy()

      # 1. Remove old placement
      old_support = new_placements.get(pick)
      if old_support:
        # 그 아래 블록은 위가 비게 됨
        if old_support != "table":
          new_top_status[old_support] = True
        del new_placements[pick]

      # 2. Add new placement
      if place in placements or place in top_status:
        # place가 블록/볼이면
        new_placements[pick] = place
        new_top_status[place] = False
      else:
        # place가 on-table location이면 그냥 table로 normalize
        new_placements[pick] = place
        new_top_status[place] = False

      # 3. pick 자체는 여전히 empty on top
      new_top_status[pick] = True

      return new_placements, new_top_status, True, "OK"

    def state_to_natural_language(placements, top_status):
      """
      Convert placements + top_status dicts into natural language sentences.
      """
      lines = []
      for block, support in placements.items():
        if support == "table":
          lines.append(f"The {block} is placed on top of the table.")
        else:
          lines.append(f"The {block} is placed on top of the {support}.")
        if top_status[block]:
          lines.append(f"The {block} is empty on top.")
        else:
          lines.append(f"The {block} is not empty on top.")
      return lines

    # ------------------------
    # 3. Run Entire Plan
    # ------------------------
    def run_plan(plan_xml: str, initial_state: str, out_file: str = "plan_simulation_log.txt"):
      placements, top_status = parse_initial_state(initial_state)

      root = ET.fromstring(plan_xml)
      apacks = root.find("sequence").findall("apack")

      log_lines = []

      def log_print(text=""):
        print(text)  # 터미널 출력
        log_lines.append(text)  # 파일 저장용 버퍼에 추가

      log_print("=== Initial State ===")
      for line in state_to_natural_language(placements, top_status):
        log_print(line)
      log_print()
      log_print("=== User Instruction ===")
      log_print(initial_prompt)
      log_print()
      log_print("=== Found Object ===")
      log_print(found_objects)
      log_print()

      for i, apack in enumerate(apacks, 1):
        placements, top_status, feasible, reason = apply_apack(apack, placements, top_status)

        log_print(f"--- apack {i} ---")
        log_print(xml_print(apack))
        log_print()
        log_print(f"Feasible: {feasible} | Reason: {reason}")
        ...
        if not feasible:
          log_print("❌ Stopping due to infeasible action.")
          break

      # 결과 저장
      Path(out_file).write_text("\n".join(log_lines), encoding="utf-8")
      print(f"\n✅ Simulation log saved to: {out_file}")

    run_plan(state["xml_plan"], state["initial_state"])
    print(f"✅ Verified plan saved to: plan_simulation_log.txt")
    ...
    log_path = Path("./plan_simulation_log.txt")
    ...
    verified_text = (resp.text or "").strip()
    print("\nvalidation result: ", verified_text)

    ##### 계획이 올바르거나 최대 재계획 횟수를 초과할 경우 그대로 plan_execution_node로 이동
    if "task completion: success" in verified_text.lower() or state["validation_count"] >= 3:
      state["plan_mode"] = "replan"
    ##### 계획이 올바르지 않을 경우 error_list에 오류 사항을 채워서 planning_node로 이동 후 재계획
    else:
      state["validation_count"] += 1
      state["error_list"] = verified_text
      state["plan_mode"] = "revise"
...
# (1) planning에서의 분기 로직을 함수로 정의
def route_from_planning(state: SystemState) -> str:
    return "plan_execution" if state.get("plan_mode") == "replan" else "validation"

# (1) validation에서의 분기 로직을 함수로 정의
def route_from_validation(state: SystemState) -> str:
  return "plan_execution" if state.get("plan_mode") == "replan" else "planning"

# (1) plan_execution에서의 분기 로직을 함수로 정의
def route_from_execute(state: SystemState) -> str:
    # 최종 종료 조건
    if state.get("done"):
        return "end"
    # 그 다음 실패 분기
...
if __name__ == "__main__":
    ...
    initialState: SystemState = {
        ...
        "current_state": "",
        "domain_rules": Domain_3_rules,
        "example": Domain_3_example,
        "error_list": "",
